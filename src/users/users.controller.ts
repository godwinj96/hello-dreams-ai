...
export class UsersController {
  ...
  getUserById(@Request() req, @Param('id', UUIDValidationPipe) id: string): Promise<UserResponseDto> {
    return this.usersService.getUserById(req.user.id, id);
  }

  @Post('admins')
  @Roles(Role.Superuser)
  @ApiOperation({ summary: 'Create admin or superuser account (superuser only)' })
  @ApiBody({ type: CreateAdminDto })
  @ApiResponse({ status: 201, description: 'Admin/superuser created successfully', type: UserResponseDto })
  @ApiResponse({ status: 403, description: 'Only superusers can create admins/superusers' })
  ...
  @Patch(':id/promote')
  @Roles(Role.Superuser)
  @ApiOperation({ summary: 'Promote user to admin or superuser (superuser only)' })
  ...
  @ApiResponse({ status: 403, description: 'Only superusers can promote users' })
  @ApiResponse({ status: 404, description: 'User not found' })
  ...
  @ApiResponse({ status: 403, description: 'Only admins can update user status' })
  @ApiResponse({ status: 404, description: 'User not found' })
